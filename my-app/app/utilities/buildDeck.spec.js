...
test('Returns a sequence that matches the length of the array passed', () => {
  ...
  expect(buildDeck(['ace:heart','ace:heart','ace:heart','ace:heart','ace:heart']).sequence.length).toEqual(5);
});

test('Returns meta with all cards set to face down', () => {
  const deck = buildDeck(['ace:heart','two:heart','three:heart','four:heart','five:heart']);
