...
import { PilesInterface, PileIdsInterface, CardInterface, CardFaceInterface, SuitInterface } from "@/app/types/solitaire.types";
import {
...
export default function WastePile() {
  ...
  const wasteDoubleClickHandler = (event: React.BaseSyntheticEvent, card: CardInterface) => {
    const suit = card.split(":")[1] as SuitInterface;
    const targetBuildPile: PileIdsInterface = `build_${suit}`;
    if (
      canCardBeAddedToBuildPile(
        card,
        state[targetBuildPile].sequence,
        suit,
      )
    ) {
      dispatch({
